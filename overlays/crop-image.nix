...
delib.overlayModule {
    name = "crop-image";
    overlay = final: prev: {
        crop-image = final.writers.writePython3Bin "crop-image" {doCheck = false;} ''
            import argparse
            import os
            import re
            import subprocess
            ...
            def main():
                ...
                result = subprocess.run(
                    ['${final.imagemagick}/bin/identify', '-format', '%w %h', input_path],
                ...
                subprocess.run([
                    '${final.imagemagick}/bin/magick', input_path,
