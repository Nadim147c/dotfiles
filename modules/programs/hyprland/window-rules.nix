...
delib.module {
    ...
    options = delib.moduleOptions (with delib; {
        layerRules = lib.mkOption {
            type = lib.types.attrsOf (lib.types.listOf str);
            default = {
                blur = ["logout_dialog" "launcher" "rofi" "wofi"];
                ignorezero = ["launcher" "gtk-layer-shell" "wofi"];
                noanim = [
                    "selection"
                    "hyprpicker"
                    "mpvpaper"
    ...
    home.ifEnabled = {cfg, ...}: {
        wayland.windowManager.hyprland.settings.layerrule = let
            joinKeyVals = key: vals: map (v: "${key},${v}") vals;
        in
            cfg.layerRules |> lib.mapAttrsToList joinKeyVals |> builtins.concatLists;
