...
delib.script {
    name = "crop-image";
    package = pkgs.writers.writePython3Bin "crop-image" {doCheck = false;} ''
        import argparse
        import os
        import re
        import subprocess
        ...
        def main():
            ...
            result = subprocess.run(
                ["${pkgs.imagemagick}/bin/identify", '-format', '%w %h', input_path],
            ...
            subprocess.run([
                '${pkgs.imagemagick}/bin/magick', input_path,
